...
add_data = """{
  "data": {
    ...
    {"student_name": "test string", "major": "test string", "degree": "test string"}
         ,
...
class TestStudents(unittest.TestCase):
    ...
    def test_01_add(self):
        ...
        rv = self.app.post('/api/v1/students.json',
                           data=add_data, content_type="application/json")
        assert rv.status_code == 201

    def test_02_read_update(self):
        request = self.app.get('/api/v1/students.json')
        dict = json.loads(request.data.decode('utf-8'))
        id = dict['data'][0]['id']
        rv = self.app.patch('/api/v1/students/{}.json'.format(id),
    ...
    def test_03_delete(self):
        ...
        dict = json.loads(request.data.decode('utf-8'))
        id = dict['data'][0]['id']
        rv = self.app.delete('/api/v1/students/{}.json'.format(id))
